...
本文先交代问题与方法，再按时间顺序给出两套人类标注结果。两套结果用了同一种切法（\(S_{end}\) + q80/q20），但**窗向量的构造不同**，分数高低不能直接比较。

| | 窗向量 | 样本 | 图 |
|---|---|---|---|
| **第一部分** | 一窗 10 条评论拼成一段长文本再嵌（concat） | 200 条试点 | 图 1–7 |
| **第二部分** | 每条评论先嵌，再在窗内平均（mean-pool） | 已打分的 650 条（全量约 1900 的前一段） | 图 1–7；其中图 6–7 只含两端各 130 条 |
...
一窗 10 条评论要变成**一个**向量，报告里试了两种做法（后文第一部分 / 第二部分）：

- **Concat（第一部分）**：把 10 条正文拼成一段长文本，再嵌一次。长评会主导这一窗。  
- **Mean-pool（第二部分）**：10 条各嵌一次，再平均。每条评论票权相同。

两种都走后面的「相对第一窗画曲线 → \(S_{end}\) → q80/q20」。**分数高低不能直接比**，因为向量的做法变了。
...
| Vendi | 后半段内部还有多少种意思（不问像不像开头） | 辅证。concat 试点和 mean-pool 650 两端都算过；不锁主指标 |
...
# 第一部分：Concat 窗向量 · 200 条试点

> 本部分全部数字和图，都来自「10 条评论拼成一段再嵌」。不要和第二部分 mean-pool 的 0.88 / 0.81 比谁更大。
...
4. **q80/q20 比 q70/q30 类间更干净**：更适合当后续对照实验的种子池。  
...
在 200 条真人帖上，用后期相对开场的语义相似度作为主分数，按最高、最低各 20% 切开后，收敛类平均约 0.77，发散类平均约 0.52，中间 60% 暂不进入主实验。这说明人类内部存在可分的相对两端，足以支撑下一步多智能体对照。它还不是人机对比的最终结论，也和参考论文里 LLM 与人类的数量级差异不是同一回事。

---

## 六（续）：三个候选指标 + 七张图（仍是 concat 200）
...
**说明了什么：**  
若问「后期还像不像开头」，平均距离和 \(S_{end}\) 是同一家族：一个看结尾有多像，一个看一路离开多远。所以它最能分开两类，**并不等于发现了第三种完全不同的现象**。AI 示意与肉眼一致：多智能体比人类更黏开场。这仍是 200 条试点，先记下，不锁成唯一主指标。

图 5 是图 1 那条轨迹的另一种摘要（离开有多远），不是另一套实验。
...
**说明了什么：**  
「离开开头」和「后半段内部多样」不是一回事。好比一桌人可以从「今天吃什么」聊到「房租、球赛、前任」——离开了开头（图 5 会变大），但后半段内部可能仍然各说各话、花样很多，也可能已经只剩一种腔调。人类不收敛帖主要是走开了，不一定是后半段内部更热闹。Vendi 不太适合当人类收/散的主分类，但能看出 AI 后半段内部更单一。所以这项保留作辅证，不拿来切两类。

图 6 不问「还像不像开头」，只问后半段内部是否仍然多样。
...
这是图 1–4 的合版，内容和分图相同，只是挤在一页里。详细读图见上面的分图说明。

第一部分到此结束。下面换窗向量，**重新开始读，不要把 0.77 和 0.90 当成同一种进步**。

---

# 第二部分：Mean-pool 窗向量 · 650 条临时结果

> 切法与第一部分相同：\(S_{end}\) + q80/q20 + 蓝/红/灰。  
> 变的是第四步：每条评论先变成向量，再在窗里平均。  
> 样本是过滤后全量 1900 里已经有分数的前 650 条。标签只在这 650 条上切，全量齐了会作废重切。  
> **不要和第一部分的数字并排比大小。**

### 为什么要换窗向量

Concat 把 10 条评论粘成一篇超长作文再嵌。谁写得长，谁就更容易主导这一窗；长串也容易把这台 8GB 笔记本的 GPU 烤死。

Mean-pool 是：10 个人各说一句，各算一个向量，再取平均。每个人票权相同。几何上分数会被抬高、两端被压窄——看起来「更收了」，其实是尺子变了，不是帖子突然都变乖了。

### 这 650 条怎么来的

合格帖仍是 50–1000 条评论。分数来自三块拼起来再**切一次**标签：GPU 400 + CPU 150 + GPU 100。中间某一块自己切过的标签都丢掉了。

| 项 | 这 650 条（mean-pool） |
|---|---|
| 标签 | 130 converge / 130 non / 390 ambiguous |
| q80 / q20 | 0.887 / 0.808（带宽 0.078） |
| \(S_{end}\) | 最小 0.52，中位 0.85，最大 0.98 |
| 两类均值 | converge 0.90，non 0.76 |
| 离开约 0.85 这条中带 | 蓝 69/130 从未掉破；红全部掉破 |

#### Figure 1. 轨迹（mean-pool, n=650）

![Figure 1 mean650](figures/fig1_human_trajectories_mean650.png)

**用的指标：** 与第一部分图 1 相同，\(\cos(1,t)\)。  
**横轴 / 纵轴：** 窗序号 \(t\)；和开头有多像。

**图画了什么：** 细线是各帖轨迹，粗线是两类平均。虚横线是这 650 条自己的 q80 / q20。

**怎么读：** 先看粗线。蓝线掉到约 0.90 后横着走；红线在约 0.80–0.85。两端仍分开，但整张图贴在高位。

**说明了什么：** 换了窗向量，人类内部相对两端还在。**不能**读成「比第一部分更收敛」——尺子不同。

#### Figure 2. 平台高度（各举一例）

![Figure 2 mean650](figures/fig2_plateau_examples_mean650.png)

**用的指标：** 仍是 \(\cos(1,t)\)。虚线是该帖 \(S_{end}\)。

**怎么读：** 蓝例（Raiders / LA 球迷）停在约 0.90；红例停在约 0.76。读法与第一部分图 2 相同：看后期停多高，不看斜率是否永远向上。

**说明了什么：** 「收敛」仍然是平台高，不是曲线往上爬。

#### Figure 3. \(S_{end}\) 分布与切分

![Figure 3 mean650](figures/fig3_send_distribution_mean650.png)

**用的指标：** \(S_{end}\)。越高 = 结尾越像开头。

**图画了什么：** 灰柱 650 条；蓝 130、红 130；竖线 q20≈0.808、q80≈0.887。

**怎么读：** 仍是一座山，高峰约 0.85，不是左右两团。切的是这座山的两端。

**说明了什么：** 相对分类在更大样本上仍然成立；切线数字已与第一部分不同，这是预期。

#### Figure 4. 离开时间

![Figure 4 mean650](figures/fig4_departure_time_mean650.png)

**用的指标：** 第一次 \(\cos(1,t)\) 掉到约 0.85（q20 与 q80 中点）以下的窗号。

**怎么读：** 红类全部掉破，多半较早；蓝类 69 条从未掉破。切线本身比第一部分更高，所以「从未掉破」的蓝帖会变多。

**说明了什么：** 散的帖往往更早离开开头。具体窗号不要和第一部分的 0.66 阈值去对表。

#### Figure 5. 平均 converge distance

![Figure 5 mean650](figures/fig5_avg_converge_distance_mean650.png)

**用的指标：** 第一窗之后各窗 \(1-\cos(1,t)\) 再平均。越大 = 一路离开开头越远。

**图画了什么：** 左蓝 130、右红 130。没有叠第一部分的 AI 点（那是 concat 种子）。

**怎么读：**

| | 图 3 的 \(S_{end}\) | 图 5 的平均距离 |
|---|---|---|
| 问的问题 | **结尾**还像不像开头 | **一路上**平均离开多远 |
| 越大表示 | 越黏开头 | 越离开开头 |
| 这 650 条 | 0.90 vs 0.76 | 0.10 vs 0.18 |

两类分得很开，是因为距离和用来打标签的 \(S_{end}\) 是同一条轴的正反面。

**说明了什么：** 图 5 帮人建立「距离 = 不像开头」的直觉，**不是新的独立发现，也不锁主指标**。

#### Figure 6. 后期 Vendi（mean-pool 650 的两端）

![Figure 6 mean650](figures/fig6_vendi_late_mean650.png)

**用的指标：** 后期窗 Vendi。看**后半段内部**还有多少种有效意思，不问「和开头像不像」。  
**越大** = 后半段内部越多样。

**横轴 / 纵轴：** 人类类别（这 650 条上的 q80/q20）vs Vendi。没有叠第一部分的 AI 点。

**图画了什么：** 左蓝 130、右红 130。数据来自对这两端帖**再嵌一遍每条评论**（先 130 converge，停 10 分钟，再 130 non）。

**怎么读：** 蓝均值约 0.50，红均值约 0.45，差别不大（Cohen’s d≈0.39）。散的帖并不明显「后半段更热闹」。

**说明了什么：** 和第一部分图 6 同一句话——「离开开头」≠「后半段内部多样」。人类 non_converge 主要是走开了，不是后半段自己更吵。这项仍不适合当主分类。全量 1900 重切后数字会变，不锁。

图 6 只问后半段内部是否仍然多样。

#### Figure 7. 有效维度（mean-pool 650 的两端）

![Figure 7 mean650](figures/fig7_effective_dim_mean650.png)

**用的指标：** Effective dim（有效维度 / 参与比）。后期评论占了多少个有效语义方向。  
**越大** = 后期话题方向越多。

**图画了什么：** 读法同图 6，同一批 130+130。

**怎么读：** 蓝比较集中，均值约 3.08；红均值约 3.32，但更散。差别弱（d≈0.31），红略高。

**说明了什么：** 「不收敛 = 方向更多」只是弱趋势。不适合单独当人类分类主指标。不要和第一部分图 7 的 3.16 / 3.56 比谁更扁——样本和窗向量都不同。

图 7 问后半段占了几个意思方向，不是「和开头像不像」。

合图（第二部分图 1–4）：

![2x2 mean650](figures/human_trajectories_mean650_2x2.png)

标签文件：`outputs/labels/thread_labels_mean650.jsonl`（临时）。
...
| 一窗的向量怎么来 | 论文自己的构造 | 试了 concat（第一部分）和 mean-pool（第二部分），**两套分数不混比** |
...
1. **把过滤后全量 1900 条的 mean-pool 分数跑完**，再切一次 q80/q20（现在的 650 条标签作废）。  
2. **每类抽约 50 帖**做多智能体基线。  
...
3. **报告里有两套窗向量（concat 200、mean-pool 650），数字不能混比；全量 1900 之后还会再切一次。**  
4. **Phase-1 目前证明的是“人类两端可分 + 流水线可用”，还不是最终人机对比结论。**  
5. **和参考论文同轴不同问：他们重人机鸿沟与闭环顽固性；我们重人类分流与可复现基线。**
...
| 试跑标签（concat，q80/q20） | `outputs/labels/thread_labels_q80q20.jsonl` |
| mean-pool 650 临时标签 | `outputs/labels/thread_labels_mean650.jsonl` |
| 试跑摘要 | `outputs/labels/label_summary_q80q20.json` |
| 默认配置 | `configs/default.yaml` |
| 人类轨迹 2×2 合图（concat 200） | `outputs/figures/human_trajectories_200_2x2.png` |
| mean-pool 650 分图 | `outputs/figures/fig1_*_mean650.png` … `fig7_*_mean650.png` |
| 分图 1–7（concat 试点，报告前半） | `outputs/figures/fig1_*.png` … `fig7_*.png` |
